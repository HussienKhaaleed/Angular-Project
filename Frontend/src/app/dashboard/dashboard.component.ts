...
import { TmdbService, MovieItem, Genre } from '../services/tmdb.service';
...
export class DashboardComponent implements OnInit {
  ...
  loadTrendingMovies(): void {
    ...
    this.tmdb.discoverMovies({ page: 1 }).subscribe({
      next: (movies) => {
  ...
  loadTopRatedMovies(): void {
    ...
    this.tmdb.discoverMovies({ page: 2 }).subscribe({
      next: (movies) => {
  ...
  loadNowPlayingMovies(): void {
    ...
    this.tmdb.discoverMovies({ page: 3 }).subscribe({
      next: (movies) => {
  ...
  getVoteColor(voteAverage: number): string {
    if (voteAverage >= 8) return '#22c55e';
    if (voteAverage >= 7) return '#3b82f6';
    if (voteAverage >= 6) return '#eab308';
    return '#ef4444';
  }
